...
//		}
		...
		static void PushChar(Font* font, Uint16 c, const Engine::Math::vec2& pos, const Engine::Math::vec4& color, float scale, float* x) {
			...
			R2D_PushVertex({xpos    , ypos + h, u     , v     , color.x, color.y, color.z, color.w});
			R2D_PushVertex({xpos    , ypos    , u     , v + sv, color.x, color.y, color.z, color.w});
			R2D_PushVertex({xpos + w, ypos    , u + su, v + sv, color.x, color.y, color.z, color.w});
			R2D_PushVertex({xpos    , ypos + h, u     , v     , color.x, color.y, color.z, color.w});
			R2D_PushVertex({xpos + w, ypos    , u + su, v + sv, color.x, color.y, color.z, color.w});
			R2D_PushVertex({xpos + w, ypos + h, u + su, v     , color.x, color.y, color.z, color.w});
